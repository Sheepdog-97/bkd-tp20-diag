...
| ABS 03 | TP2.0 setup discovered; KWP sequence not solved yet |
| Gateway/cluster/HVAC/body | Profiled only; active support not proven |
| Airbag/immobilizer/steering | Do not actively probe unless you understand the risk |
...
See `docs/TRACE_CAPTURE.md` and `docs/SPLITTER_CAPTURE_CHECKLIST.md`.

## Experimental modules

Active non-engine probing is gated:

```bash
sudo python3 -m bkd_diag.cli --iface can0 --experimental-module discover-session 03
sudo python3 -m bkd_diag.cli --iface can0 --experimental-module probe-module 03
```

Without `--experimental-module`, these commands stop before CAN setup.
