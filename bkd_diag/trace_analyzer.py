...
def _is_setup_or_control_frame(frame: TraceFrame) -> bool:
    data = frame.data
    if not data:
        return False
    if frame.can_id == 0x200 and len(data) >= 7 and data[1] == 0xC0:
    ...
    if len(data) >= 7 and data[1] == 0xD0:
        return True
    first = data[0]
    return first in (0xA0, 0xA1, 0xA3, 0xA8) or 0xB0 <= first <= 0xBF


def _is_tp_data_frame(data: bytes) -> bool:
    if len(data) < 4:
        return False
    return (data[0] & 0xF0) in (0x10, 0x20)


def _declared_tp_len(data: bytes) -> int:
    return ((data[1] & 0x7F) << 8) | data[2]


def _complete_reassembled_event(state: _TpAssembly, raw: str, complete: bool = True) -> TraceEvent:
    payload = bytes(state.payload[:state.expected_len])
    detail, service, service_id, subfunction = describe_kwp(payload)
    status = "reassembled" if complete else "partial"
    return TraceEvent(
        state.start_line,
        state.can_id,
        payload,
        "kwp" if complete else "kwp-partial",
        f"TP2.0 {status} KWP message seq={state.start_first_byte & 0x0F:X} "
        f"frames={state.frame_count} len={len(payload)}/{state.expected_len}: {detail}",
        raw,
        service=service,
        service_id=service_id,
        subfunction=subfunction,
        sequence=state.start_first_byte & 0x0F,
    )
...
def analyse_trace(path: str | Path) -> tuple[list[TraceFrame], list[TraceEvent], Counter[int]]:
    ...
    assemblies: dict[int, _TpAssembly] = {}
    ...
    for frame in frames:
        ...
        if _is_setup_or_control_frame(frame):
            event = classify_frame(frame)
            if event is not None:
                events.append(event)
            continue

        if _is_tp_data_frame(data):
            state = assemblies.get(frame.can_id)
            first = data[0]

            if state is None:
                expected_len = _declared_tp_len(data)
                # TP2.0 KWP lengths seen in these traces are small. If this
                # byte pattern is not a plausible first segment, fall back to
                # the legacy single-frame classifier rather than inventing a
                # huge message from continuation bytes.
                ...
                state = _TpAssembly(
                    ...
                    can_id=frame.can_id,
                    start_raw=frame.raw,
                    start_first_byte=first,
                    expected_len=expected_len,
                    payload=bytearray(data[3:]),
                )
                if len(state.payload) >= state.expected_len:
                    events.append(_complete_reassembled_event(state, state.start_raw, complete=True))
                else:
                    assemblies[frame.can_id] = state
                continue

            # Existing in-progress message for this CAN ID: this frame is a
            # continuation/final segment, regardless of whether its high nibble
            # is 0x20 or 0x10.
            state.payload.extend(data[1:])
            state.frame_count += 1
            if len(state.payload) >= state.expected_len:
                events.append(_complete_reassembled_event(state, state.start_raw, complete=True))
                assemblies.pop(frame.can_id, None)
            continue
    ...
    for state in assemblies.values():
        events.append(_complete_reassembled_event(state, state.start_raw, complete=False))
