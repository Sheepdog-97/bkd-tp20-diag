...
def print_dtc_response(
    reporter: Reporter,
    resp: bytes,
    db: DtcDatabase,
    title: str = "Fault read result",
    ecu_label: str = "engine ECU",
) -> None:
    c = reporter.colour
    ...
    if count == 0:
        reporter.ok(f"No DTCs reported by {ecu_label} for this read mode")
    ...
    expected_record_bytes = count * 3
    if len(records) < expected_record_bytes:
        reporter.warn(
            f"DTC response says {count} record(s), but only {len(records)} record byte(s) were present."
        )
    ...
    for idx in range(0, min(len(records), expected_record_bytes), 3):
