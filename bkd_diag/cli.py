...
from .module_probe import resolve_module_address, run_read_only_probe
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        experimental_actions = ("discover-setup", "discover-session", "probe-module")
        ...
        if args.action == "probe-module":
            logical_address, _module_profile = resolve_module_address(args.module)

        ecu = TP20KWP(iface=args.iface, reporter=reporter, logical_address=logical_address)
        start_session = not (args.action == "probe-module" and args.no_session_open)
        ecu.open(session=session, start_session=start_session)
